...
def gridSearchAndSave(X, y, searchParams, is_pipeline=True):
    for idx in range(len(searchParams)):
        ...
        pickle.dump(grid_clf, open('gridModels/'+ params[2], 'wb'))
        

def getGridSearchModel(filename):
    return pickle.load(open('gridModels/'+ filename, 'rb'))
...
def getIntegerEncoder(df=None):
    encoderFile = Path("integerEncoder.pickle")
    if df is None and encoderFile.is_file():
        return pickle.load(open('integerEncoder.pickle', 'rb'))
    elif df is not None:
        encoder = IntegerEncoder(df)
        pickle.dump(encoder, open('integerEncoder.pickle', 'wb'))
        return encoder
    else: 
        raise ValueError('Data frame with tweet column is required')
...
if __name__ == "__main__":
    ...
    encoder = getIntegerEncoder(dataFrame)
